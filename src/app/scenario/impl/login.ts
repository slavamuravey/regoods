...
import type { LoginParams, LoginScenario } from "../login";
...
export class LoginScenarioImpl implements LoginScenario {
  constructor(
  ...
  async* login({ phone, gender, browser, proxy, userAgent, headless, quit}: LoginParams): AsyncGenerator<StepMessage> {
    ...
    try {
      ...
      if (!isAccountEmpty) {
        throw new ScenarioError(`account "${phoneResolved}" is not empty.`);
      }
